...
export default function ProductsPage() {
  return (
    <div className="flex flex-col">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-end">
          <div>
            <h1 className="font-display text-5xl md:text-7xl leading-[0.95] mb-4">
              What the ecosystem
              <br />
              <span className="text-white/30">has shipped.</span>
            </h1>
          </div>
          <div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 flex flex-col gap-5">
        {products.map((product, i) => (
          <div
            key={i}
            className={`panel-border grid grid-cols-1 md:grid-cols-2`}
          >
            {/* Image */}
            <div
            ...
            >
              <div className="absolute top-4 left-4 z-10">
                <span
                  className={`font-heading text-[9px] tracking-[0.15em] uppercase px-2.5 py-1 ${
                    product.status === "Live"
                      ? "text-emerald-400"
                      : "text-white/30"
                  }`}
            ...
            >
              <p className="label-section mb-2">{product.kind}</p>
              <h2 className="font-heading text-3xl md:text-4xl mb-4 mt-1">
                {product.name}
              </h2>
              <p className="font-sans text-white/55 text-sm leading-relaxed mb-3">
                {product.description}
              </p>
              <p className="font-sans text-white/20 text-xs leading-relaxed italic mb-10">
                {product.note}
              </p>
              {product.ctaLabel && product.href && (
                product.external ? (
                  ...
                  >
                    {product.ctaLabel} <ArrowUpRight size={10} />
                  </a>
                ) : (
                  <Link href={product.href} className="btn-ghost-ku w-fit">
                    {product.ctaLabel} <ArrowRight size={10} />
                  </Link>
                )
              )}
              {product.status === "Coming Soon" && (
                <p className="font-sans text-white/20 text-xs tracking-wide">
      ...
      <section className="bg-[#060606] py-16">
        <div className="max-w-7xl mx-auto px-6">
          <p className="label-section mb-8">— Kusho Apparel</p>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div className="md:col-span-2 relative h-64 md:h-96 overflow-hidden group panel-border">
              <Image
            ...
            {[
              ...
              <div key={i} className="relative h-64 md:h-96 overflow-hidden group bg-black panel-border">
                <Image
                  src={src}
                  alt="Kusho Merch"
                  fill
          ...
          <div className="mt-6">
            ...
            >
              Visit the Shop <ArrowUpRight size={10} />
            </a>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="border border-[#FF6B2B]/15 p-10 md:p-14 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <div>
            ...
            <p className="font-sans text-white/40 text-sm max-w-md leading-relaxed">
          ...
          <div className="flex flex-wrap gap-4">
            <Link href="/get-involved" className="btn-ku">
              Builder Lane <ArrowRight size={11} />
            </Link>
            <Link href="/labs" className="btn-ghost-ku">
              Malaya Labs →
            </Link>
          </div>
        </div>
